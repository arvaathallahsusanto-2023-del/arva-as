...
export default function RegisterStakeholder() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setStatus("success");
      setTimeout(() => setLocation("/ecosystem"), 2000);
    } catch (err: any) {
          ...
          <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input required name="name" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/50 focus:bg-white/10 transition-colors" placeholder="Cth: Dr. Ahmad" />
              </div>
              <div className="space-y-2">
                <label className="text-xs tracking-wider text-gray-400 font-semibold uppercase">Email Kontak</label>
                <input required type="email" name="email" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/50 focus:bg-white/10 transition-colors" placeholder="email@contoh.com" />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-2">
                ...
                <input required name="role" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/50 focus:bg-white/10 transition-colors" placeholder="Cth: Dosen / Direktur" />
              </div>
            </div>

            <div className="space-y-2">
              <label className="text-xs tracking-wider text-gray-400 font-semibold uppercase">Asal Institusi</label>
              <input required name="institution" className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/50 focus:bg-white/10 transition-colors" placeholder="Cth: Universitas Indonesia / Bank X" />
            </div>

            <div className="space-y-2">
              <label className="text-xs tracking-wider text-gray-400 font-semibold uppercase">Bidang Minat / Bio Singkat</label>
              <textarea name="bio" rows={3} className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 placeholder:text-gray-600 focus:outline-none focus:border-cyan-500/50 focus:bg-white/10 transition-colors resize-none" placeholder="Deskripsikan fokus riset atau bisnis Anda..." />
            </div>
